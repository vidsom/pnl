...
for count , element in enumerate(segment):
    ...
    for line in case:
        ...
        gray_gk=np.concatenate((id[0:len(id)]))
    
        
        average_gray_gk = np.mean(gray_gk)
        average_gray_all = np.mean(gray_gk)
        gk_all.append(average_gray_all)
        if casenumber.startswith('case1'):
            gk_array.append(average_gray_gk)
            sub = ('ADHD')
            subject.append(sub)
        else:
            gk_array1.append(average_gray_gk)
            sub1 = ('Control')
            subject.append(sub1)
    ...
    data={'Values': gk_all,'Group': subject }
    
    d = pd.DataFrame(data)
    ROI=region[count]
    roi = segment_name[count]
    
    t, s = (stats.ttest_ind(gk_array, gk_array1, equal_var = False))
    print(stats.ttest_ind(gk_array, gk_array1, equal_var = False))
    
    t_statistic.append(t)
    p_value.append(s)

    from math import sqrt
    cohens_d = (np.mean(gk_array) - np.mean(gk_array1)) / (sqrt((np.std(gk_array) ** 2 + np.std(gk_array1) ** 2) / 2))
    print(cohens_d)
    ...
    sns.plt.title('{0}: GK'.format(ROI), size=20)
    plt.xlabel("Group", size = 16)
    plt.ylabel("GK Values", size = 16)
#    plt.savefig('/rfanfs/pnl-zorro/home/vidushi/ADHD_MSD_FW/new_graphs/{0}_gk.png' .format(roi), bbox_inches = 'tight')
    plt.show()
