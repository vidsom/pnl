...
for count , element in enumerate(segment):
    ...
    for line in case:
        ...
        gray_rtap=np.concatenate((id[0:len(id)]))
        gray_msd=np.concatenate((id_msd[0:len(id_msd)]))
        
        
        in_l = (gray_msd <= 15)
        keep = np.where(in_l)
        final_gray_rtap = gray_rtap[keep]
    
        
        average_gray_rtap = np.mean(final_gray_rtap)
        average_gray_all = np.mean(final_gray_rtap)
        rtap_all.append(average_gray_all)
        if casenumber.startswith('case1'):
            rtap_array.append(average_gray_rtap)
            sub = ('ADHD')
            subject.append(sub)
        else:
            rtap_array1.append(average_gray_rtap)
            sub1 = ('Control')
            subject.append(sub1)
    ...
    data={'Values': rtap_all,'Group': subject }
    
    d = pd.DataFrame(data)
    ROI=region[count]
    roi = segment_name[count]
    
    t, s = (stats.ttest_ind(rtap_array, rtap_array1, equal_var = False))
    print(stats.ttest_ind(rtap_array, rtap_array1, equal_var = False))
    
    t_statistic.append(t)
    p_value.append(s)
    
    from math import sqrt
    cohens_d = (np.mean(rtap_array) - np.mean(rtap_array1)) / (sqrt((np.std(rtap_array) ** 2 + np.std(rtap_array1) ** 2) / 2))
    print(cohens_d)
    ...
    sns.plt.title('{0}: RTAP'.format(ROI), size=20)
    plt.xlabel("Group", size = 16)
    plt.ylabel("RTAP Values", size = 16)
#    plt.savefig('/rfanfs/pnl-zorro/home/vidushi/ADHD_MSD_FW/new_graphs/{0}_rtap.png' .format(roi), bbox_inches = 'tight')
    plt.show()    
