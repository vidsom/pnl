...
for count , element in enumerate(segment):
    ...
    for line in case:
        ...
        average_gray_msd = np.mean(ultimate_gray_msd)
        average_gray_all = np.mean(ultimate_gray_msd)
        msd_all.append(average_gray_all)
        if casenumber.startswith('case1'):
            msd_array.append(average_gray_msd)
            sub = ('ADHD')
            subject.append(sub)
        else:
            msd_array1.append(average_gray_msd)
            sub1 = ('Control')
            subject.append(sub1)
    ...
    data={'Values': msd_all,'Group': subject }
    
    d = pd.DataFrame(data)
    ROI=region[count]
    roi = segment_name[count]
    
    t, s = (stats.ttest_ind(msd_array, msd_array1, equal_var = False))
    print(stats.ttest_ind(msd_array, msd_array1, equal_var = False))
    ...
    cohens_d = (np.mean(msd_array) - np.mean(msd_array1)) / (sqrt((np.std(msd_array) ** 2 + np.std(msd_array1) ** 2) / 2))
    print(cohens_d)
    ...
    sns.plt.title('{0}: MSD'.format(ROI), size=20)
    plt.xlabel("Group", size = 16)
    plt.ylabel("MSD Values", size = 16)
#    plt.savefig('/rfanfs/pnl-zorro/home/vidushi/ADHD_MSD_FW/new_graphs/{0}_msd.png' .format(roi), bbox_inches = 'tight')
    plt.show()    
