...
for count , element in enumerate(segment):
    ...
    for line in list_adhd:
        ...
        attnprob_T.append(float(measure))
        
    list_adhd.close()
    
    
    
    data={'MSD': msd_array_adhd, 'ADHD': attnprob_T}
    d = pd.DataFrame(data)
    ...
    sns.plt.title('MSD and attnprob_T Correlation: {0}' .format(ROI), size = 21)
    ...
    sns.plt.xlabel("attnprob_T", size = 16)
